...
Run over a second text — **Euler's *Elements of Algebra*** (627 leaves, 20,431 expressions checked) — the same pipeline surfaced a different kind of error, one Euler made himself. In the chapter on imaginary quantities he writes `sqrt(-2) · sqrt(-3) = sqrt(6)` (and `sqrt(-1) · sqrt(-4) = 2`). The correct value is `−sqrt(6)`: the rule `sqrt(a)·sqrt(b) = sqrt(ab)` does not hold once the roots go imaginary. This is a genuine mathematical mistake, famous enough that it appears in histories of complex numbers, and the numeric checker flags it without being told to look. Two texts, two different failure modes caught by the same four checks — a faithfully-transcribed printing error and a real author error — see [`euler_algebra_errata.json`](euler_algebra_errata.json).

The rest of what the Algebra run flags is a lesson in what the checker is *for*. Of 492 equalities it could not prove, the honest triage is: 332 are equations being solved or definitions (they hold under local hypotheses no checker can see), 80 are decimals verified against their printed digits, and the remainder are conventions, series, and provable identities. That leaves ~47 pure-numeric mismatches beyond the imaginary-number error — and those are mostly the checker catching **its own single-digit transcription slips** (`33² − 13 = 1010`, where `33² − 13 = 1076`). That is the point: on 20,184 expressions the tool hands a human the ~50 lines worth re-reading and labels why, instead of asserting Euler made fifty mistakes he did not.
